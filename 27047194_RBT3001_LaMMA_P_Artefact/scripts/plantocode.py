...
class LLMHandler:
    ...
    def setup_api(self, api_key_file: str) -> None:
        ...
        try:
            ...
            for api_key_path in (Path(api_key_file + '.txt'), Path(api_key_file)):
            ...
            if self.base_url:
                ...
                print(f"Using local/OpenAI-compatible API base URL: {self.base_url}")
                return

            if not api_key:
                raise LLMError(
                    f"API key file not found: {api_key_file}. For local models, pass --base-url "
                    "or set LOCAL_LLM_BASE_URL/OPENAI_BASE_URL; a dummy key will be used."
                )
...
class MimicFormatTranslator:
    ...
    def __init__(self, api_key_file: str, gpt_version: str = "gpt-4o", base_url: Optional[str] = None):
...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Translate complete PDDL plans to AI2-THOR executable code using an OpenAI-compatible API. Can load from JSON files or PDDL log directories created by pddlrun_llmseparate.py')
    parser.add_argument('--openai-api-key-file', type=str, default="api_key",
                       help='Path to API key file. Optional when --base-url points at a local service.')
    parser.add_argument('--gpt-version', type=str, default="gpt-4o",
                       help='Model name to use, e.g. gpt-4o, llama3.1:8b, qwen2.5:7b-instruct')
    parser.add_argument('--base-url', type=str, default=None,
                       help='OpenAI-compatible base URL for Open WebUI/Ollama/local models')
